(() => {
  ...
  const state = { user: null, role: "", catalogs: {}, current: "dashboard", openOrderId: null, orderDetail: null, orderStep: "resumen", notifications: [], notificationFilter: "TODAS", unreadNotifications: 0, account: null, masterCatalog: [], catalogFilter: { q: "", categoria: "TODAS" }, userAdmin: { q: "", rol_id: "", estado: "", fecha_desde: "", fecha_hasta: "", pagina: 1, tamano: 10, orden: "FECHA", direccion: "DESC", total: 0, paginas: 1, catalogos: null }, clientSites: [] };
  ...
  const modules = {
    COORDINADOR: [
      ...
      ["mantenimientos", "settings", "Mantenimiento"], ["catalogo", "book-open-check", "Catálogo maestro"], ["cotizaciones", "file-text", "Cotizaciones"], ["documentos", "files", "Documentos"],
  ...
  async function renderUsuarios(){
    ...
    content.innerHTML=`
      ...
      <section class="panel user-report-panel">
        <div class="panel-header user-report-heading"><div><span class="section-kicker">REPORTE INTELIGENTE</span><h2>Usuarios del sistema</h2><p class="muted">Combina búsqueda, rol, estado y rango de fechas. El ordenamiento y la paginación se resuelven desde SQL Server.</p></div><button class="btn primary user-create-button" type="button" data-action="nuevo-usuario"><i data-lucide="user-round-plus"></i> Registrar usuario</button></div>
        ...
        <div class="user-report-result-header"><div><strong>${normalizeNumber(d.pagination?.total)} resultado(s)</strong><span>${f.estado?`Filtro: ${esc(f.estado.toLowerCase())}`:"Reporte general"}</span></div><span class="sql-report-chip"><i data-lucide="database-zap"></i> SQL Server · SP</span></div>
  ...
  function paintCatalogRows() {
    ...
    const rows = items.map(x => `<tr>
      ...
      <td><span class="catalog-origin ${x.origen === "SQL_SERVER" ? "db" : "custom"}">${x.origen === "SQL_SERVER" ? "Servicio BD" : "Personalizado"}</span></td>
  ...
  async function renderCatalogoMaestro(){
    ...
    const d = await api("/api/catalogo-maestro");
    ...
    content.innerHTML = `
      ...
      <section class="panel catalog-master-panel">
        <div class="panel-header catalog-master-header"><div><h2>Listado maestro</h2><p class="muted"><span id="catalogVisibleCount">${state.masterCatalog.length}</span> trabajos visibles · los servicios oficiales de SQL Server se mantienen enlazados sin cambiar la estructura de la BD.</p></div><button class="btn primary" type="button" data-action="nuevo-catalogo"><i data-lucide="plus"></i> Nuevo trabajo</button></div>
  ...
  function catalogForm(itemId=""){
    ...
    const isDb = current?.origen === "SQL_SERVER";
    ...
    openModal(current ? "Editar trabajo del catálogo" : "Nuevo trabajo del catálogo", `<form id="catalogForm" class="form-grid">
      ...
      ${isDb?`<div class="field full"><div class="catalog-db-note"><i data-lucide="database"></i><span>Este trabajo viene de <strong>srv.TipoServicio</strong>. Código y nombre se mantienen sincronizados con SQL Server; aquí administras precio, categoría y descripción sin modificar la estructura de la BD.</span></div></div>`:""}
  ...
  async function quoteForm(preselectedOrderId=""){
    ...
    let catalogItems=[];
    try{const cd=await api("/api/catalogo-maestro?solo_activos=1");catalogItems=(cd.items||[]).filter(x=>normalizeNumber(x.precio)>0);state.masterCatalog=cd.items||[];}catch(_){catalogItems=[];}
    ...
    openModal(preselectedOrderId?"Crear cotización desde la OT":"Nueva cotización simple",`<form id="quoteForm" class="form-grid">
      ...
      ${catalogItems.length?`<div class="field full"><label>Trabajo del catálogo maestro <span class="optional-label">opcional</span></label><select id="quoteCatalog"><option value="">Seleccionar para reutilizar descripción y precio...</option>${catalogItems.map(x=>`<option value="${esc(x.id)}" ${matchingCatalog?.id===x.id?"selected":""}>${esc(x.codigo)} · ${esc(x.nombre)} · ${money(x.precio,"GTQ")}</option>`).join("")}</select><small>Seleccionarlo solo precarga la propuesta; puedes ajustar el alcance y el precio antes de guardar.</small></div>`:""}
      ...
      <div class="field"><label>Precio</label><input name="precio" id="quotePrice" type="number" step="0.01" min="0" value="${matchingCatalog?normalizeNumber(matchingCatalog.precio):""}" required></div>
    ...
    $("#quoteCatalog")?.addEventListener("change",e=>{const item=catalogItems.find(x=>x.id===e.currentTarget.value);if(!item)return;const desc=$("#quoteDescription"),price=$("#quotePrice");if(desc)desc.value=item.descripcion||item.nombre;if(price)price.value=normalizeNumber(item.precio);});
