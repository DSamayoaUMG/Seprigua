...
CATALOG_FILE = DATA_DIR / "catalogo_maestro.json"
CATALOG_LOCK = Lock()
...
# La estructura SQL Server del proyecto permanece intacta. Los tipos de
# servicio existentes se leen de srv.TipoServicio y sus precios/metadatos se
# guardan en un archivo de configuración del backend.
# ---------------------------------------------------------------------------
def _catalog_default_store() -> dict:
    return {"version": 1, "items": {}}


def _catalog_load_unlocked() -> dict:
    if not CATALOG_FILE.exists():
        return _catalog_default_store()
    try:
        ...
        app.logger.exception("No se pudo leer catalogo_maestro.json")
        return _catalog_default_store()


def _catalog_save_unlocked(store: dict) -> None:
    DATA_DIR.mkdir(exist_ok=True)
...
def _catalog_payload() -> list[dict]:
    db_types = query_all(
        "SELECT TipoServicioId AS id, Codigo AS codigo, Nombre AS nombre, Activo AS activo "
        "FROM srv.TipoServicio ORDER BY Nombre"
    )
    with CATALOG_LOCK:
        store = _catalog_load_unlocked()
        meta = dict(store.get("items") or {})
    ...
    for row in db_types:
        key = f"db:{row['id']}"
        db_keys.add(key)
        extra = meta.get(key) or {}
        items.append({
            "id": key,
            "tipo_servicio_id": int(row["id"]),
            "codigo": row.get("codigo") or f"SRV-{row['id']}",
            "nombre": row.get("nombre") or "Servicio",
            "categoria": extra.get("categoria") or _catalog_category(row.get("nombre")),
            "precio": _catalog_number(extra.get("precio"), 0),
            "unidad": extra.get("unidad") or "servicio",
            "descripcion": extra.get("descripcion") or "",
            "activo": _catalog_bool(extra.get("activo"), bool(row.get("activo", 1))),
            "origen": "SQL_SERVER",
            "actualizado": extra.get("actualizado"),
        })

    for key, extra in meta.items():
        if key in db_keys or not str(key).startswith("custom:"):
            continue
        items.append({
            "id": key,
            "tipo_servicio_id": None,
            "codigo": extra.get("codigo") or "CAT",
            "nombre": extra.get("nombre") or "Trabajo personalizado",
            "categoria": extra.get("categoria") or _catalog_category(extra.get("nombre")),
            "precio": _catalog_number(extra.get("precio"), 0),
            "unidad": extra.get("unidad") or "servicio",
            "descripcion": extra.get("descripcion") or "",
            "activo": _catalog_bool(extra.get("activo"), True),
            "origen": "PERSONALIZADO",
            "actualizado": extra.get("actualizado"),
        })

    return sorted(items, key=lambda x: (not x["activo"], str(x["categoria"]).lower(), str(x["nombre"]).lower()))
...
def _catalog_validate(data: dict, *, custom: bool) -> dict:
    ...
    return {
        "codigo": codigo[:40],
        "nombre": nombre[:160],
        "categoria": str(data.get("categoria") or "General").strip()[:80],
        "precio": precio,
...
def api_catalogo_maestro(session):
    try:
        items = _catalog_payload()
        if request.args.get("solo_activos") in {"1", "true", "yes"}:
            items = [x for x in items if x["activo"]]
        categorias = sorted({x["categoria"] for x in items if x.get("categoria")})
        return jsonify(ok=True, items=items, categorias=categorias,
                       total=len(items), activos=sum(1 for x in items if x["activo"]),
...
def api_catalogo_maestro_crear(session):
    ...
    try:
        ...
        key = f"custom:{uuid.uuid4().hex[:16]}"
        with CATALOG_LOCK:
            store = _catalog_load_unlocked()
            codes = {str(v.get("codigo") or "").upper() for v in store["items"].values()}
            if item["codigo"].upper() in codes:
                raise ValueError("Ya existe un trabajo personalizado con ese código.")
            store["items"][key] = item
            _catalog_save_unlocked(store)
        return jsonify(ok=True, message="Trabajo agregado al catálogo maestro.", item={"id": key, **item, "origen": "PERSONALIZADO"}), 201
    except ValueError as exc:
        return jsonify(ok=False, message=str(exc)), 400
    except Exception as exc:
...
def api_catalogo_maestro_actualizar(session, item_id: str):
    ...
    try:
        ...
        current = next((x for x in _catalog_payload() if x["id"] == item_id), None)
        if not current:
            raise ValueError("El trabajo indicado no existe en el catálogo.")
        cleaned = _catalog_validate({**current, **data}, custom=is_custom)
        if not is_custom:
            # Código y nombre vienen del catálogo oficial srv.TipoServicio.
            cleaned["codigo"] = current["codigo"]
            cleaned["nombre"] = current["nombre"]
        with CATALOG_LOCK:
            store = _catalog_load_unlocked()
            store["items"][item_id] = cleaned
            _catalog_save_unlocked(store)
        return jsonify(ok=True, message="Catálogo actualizado.", item={"id": item_id, **cleaned, "origen": current["origen"]})
    except ValueError as exc:
        return jsonify(ok=False, message=str(exc)), 400
    except Exception as exc:
...
def api_cambiar_estado(session, orden_id: int):
    ...
    try:
        ...
        if target_code == "COMPLETADA" and session["rol"] == "TECNICO":
            closure = query_one(
                """
                SELECT
            ...
            if not str(closure.get("orden_papel") or "").strip():
                return jsonify(ok=False,message="Paso 1 pendiente: registra el número de OT u OC antes de finalizar la OT."),409
            if int(closure.get("documentos") or 0) < 1:
                return jsonify(ok=False,message="Paso 1 pendiente: adjunta la foto o PDF de la OT u OC física."),409
...
def api_mis_sedes(session):
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            set_audit_context(cursor, session, "Consulta de sedes propias del cliente")
            items = exec_proc_rows(cursor, "EXEC crm.usp_ClienteSedes_Listar ?", (session["usuario_id"],))
        for item in items:
...
def _sede_payload(data: dict) -> tuple:
    ...
    def optional_decimal(key, minimum, maximum, label):
    ...
    latitud = optional_decimal("latitud", -90, 90, "La latitud")
    longitud = optional_decimal("longitud", -180, 180, "La longitud")
...
def api_cotizaciones(session):
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            set_audit_context(cursor, session, "Listado de cotizaciones del portal")
            items = exec_proc_rows(cursor, "EXEC cot.usp_CotizacionesPortal_Listar ?", (session["usuario_id"],))
        return jsonify(ok=True, items=items)
    except Exception as exc:
...
def api_notificaciones(session):
    try:
        ...
        """,(session["usuario_id"],))
        unread=query_one("SELECT COUNT(*) AS total FROM com.Notificacion WHERE UsuarioId=? AND LeidaEn IS NULL",(session["usuario_id"],)) or {}
        return jsonify(ok=True,items=items,no_leidas=int(unread.get("total") or 0))
    ...
    try:
        summary=query_one("""
            SELECT COUNT(*) AS total,
                   SUM(CASE WHEN LeidaEn IS NULL THEN 1 ELSE 0 END) AS no_leidas
            FROM com.Notificacion WHERE UsuarioId=?
        """,(session["usuario_id"],)) or {}
        items=query_all("""
            SELECT TOP (5) NotificacionId AS id,Tipo AS tipo,Titulo AS titulo,Mensaje AS mensaje,Entidad AS entidad,
                   EntidadId AS entidad_id,Canal AS canal,Estado AS estado,CreadaEn AS creada,LeidaEn AS leida
            FROM com.Notificacion WHERE UsuarioId=? ORDER BY CreadaEn DESC
        """,(session["usuario_id"],))
        return jsonify(ok=True,total=int(summary.get("total") or 0),no_leidas=int(summary.get("no_leidas") or 0),items=items)
    except Exception as exc: return app_error(exc,"No fue posible cargar el resumen de notificaciones.")
...
def api_leer_todas_notificaciones(session):
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute("UPDATE com.Notificacion SET LeidaEn=SYSUTCDATETIME() WHERE UsuarioId=? AND LeidaEn IS NULL",session["usuario_id"])
            updated=max(cursor.rowcount,0)
            conn.commit()
        return jsonify(ok=True,message=f"{updated} notificación(es) marcada(s) como leída(s).",actualizadas=updated)
    except Exception as exc: return app_error(exc,"No fue posible actualizar las notificaciones.")
