...
def api_actualizar_cuenta(session):
    data = request.get_json(silent=True) or {}
    try:
        correo = str(require_value(data, "correo", "Ingresa el correo de la cuenta.")).strip().lower()
        if len(correo) > 254 or not CONTACT_EMAIL_RE.fullmatch(correo):
            raise ValueError("Ingresa un correo electrónico válido.")
        duplicate = query_one(
            "SELECT TOP 1 UsuarioId AS id FROM seg.Usuario WHERE Correo=? AND UsuarioId<>?",
            (correo, session["usuario_id"]),
        )
        if duplicate:
            return jsonify(ok=False, message="Ese correo ya está asociado a otro usuario."), 409
        with get_db_connection() as conn:
            cursor = conn.cursor()
            set_audit_context(cursor, session, "Actualización del correo de acceso")
            cursor.execute(
                "UPDATE seg.Usuario SET Correo=?,ActualizadoEn=SYSUTCDATETIME() WHERE UsuarioId=? AND Activo=1",
                correo, session["usuario_id"],
            )
            if cursor.rowcount == 0:
                conn.rollback()
                return jsonify(ok=False, message="La cuenta no está disponible para actualización."), 404
            cursor.execute(
                """
                INSERT INTO com.Notificacion(UsuarioId,Tipo,Titulo,Mensaje,Entidad,EntidadId,Canal,Estado)
                VALUES(?,N'SEGURIDAD',N'Correo de acceso actualizado',
                       N'El correo asociado a tu cuenta fue actualizado correctamente.',
                       N'Usuario',CONVERT(nvarchar(80),?),N'SISTEMA',N'PENDIENTE')
                """,
                session["usuario_id"],session["usuario_id"],
            )
            conn.commit()
        return jsonify(ok=True, message="Correo actualizado.", item={"correo_acceso": correo})
    except ValueError as exc:
        return jsonify(ok=False, message=str(exc)), 400
    except Exception as exc:
        return app_error(exc, "No fue posible actualizar la cuenta.")
...
def api_cambiar_contrasena(session):
    ...
    try:
        ...
        if len(nueva) > 128:
            raise ValueError("La nueva contraseña es demasiado larga.")
        if not re.search(r"[A-ZÁÉÍÓÚÑ]", nueva) or not re.search(r"[a-záéíóúñ]", nueva) or not re.search(r"\d", nueva):
            raise ValueError("La nueva contraseña debe incluir mayúscula, minúscula y número.")
        if actual == nueva:
        ...
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT ContrasenaSalt,ContrasenaHash FROM seg.Usuario WHERE UsuarioId=? AND Activo=1",
                session["usuario_id"],
            )
            row = cursor.fetchone()
            if not row:
            ...
            salt, password_hash = create_password_hash(nueva, len(bytes(row.ContrasenaHash)))
            set_audit_context(cursor, session, "Cambio de contraseña del usuario")
            cursor.execute(
                """
                UPDATE seg.Usuario
                SET ContrasenaSalt=?,ContrasenaHash=?,RequiereCambioContrasena=0,
                    IntentosFallidos=0,BloqueadoHasta=NULL,ActualizadoEn=SYSUTCDATETIME()
                WHERE UsuarioId=?
                """,
                pyodbc.Binary(salt), pyodbc.Binary(password_hash), session["usuario_id"],
            )
            cursor.execute(
                """
                INSERT INTO com.Notificacion(UsuarioId,Tipo,Titulo,Mensaje,Entidad,EntidadId,Canal,Estado)
                VALUES(?,N'SEGURIDAD',N'Contraseña actualizada',
                       N'La contraseña de tu cuenta fue actualizada correctamente.',
                       N'Usuario',CONVERT(nvarchar(80),?),N'SISTEMA',N'PENDIENTE')
                """,
                session["usuario_id"], session["usuario_id"],
            )
        ...
        return app_error(exc, "No fue posible cambiar la contraseña.")
